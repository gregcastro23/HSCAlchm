...
class RecipeAccuracyValidator:
    ...
    def generate_validation_report(self, results: Dict) -> str:
        ...
        for filename, result in results.items():
            ...
            coverage_pct = (matched_existing + truly_new) / total_pending * 100 if total_pending > 0 else 0
            report.append(f"- **Total coverage**: {matched_existing + truly_new}/{total_pending} ({coverage_pct:.1f}%)")
            report.append("")
            ...
            if comp['similarity_count'] > 0:
                ...
                for match in comp['similar_pairs'][:10]:  # Show first 10
